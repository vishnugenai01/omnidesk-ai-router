...
│   ├── student_logic.py           # ⬜ empty — Abilasha's domain
...
|---|---|---|
| **Abilasha** | Team Lead | `main.py`, `config/`, `dataaccess/`, agent/router graph, `student_logic.py`, code review on every PR |
...
│   ├── movie_router.py
│   ├── expense_router.py
│   └── agent_router.py            # Abilasha — the /ask + /logs endpoints
│
...
├── agent/
│   ├── __init__.py
│   └── agent_graph.py             # Abilasha — LangGraph StateGraph router
│
└── tests/
    ├── test_todo.py               # Sai
    ├── test_food.py               # Vishnu
    ├── test_student.py            # Abilasha
...
7. **Each person stays inside their own domain's file set** across `business_logic/`, `routers/`, `tools/`, and `tests/` — shared files (`main.py`, `config/`, `dataaccess/`, `agent/`) go through Abilasha's review since every domain depends on them.
